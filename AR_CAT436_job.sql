...
'HORIZON BCBSNJ','ANTHEM EMPIRE BCBS','ANTHEM FEP BCBS CT','ANTHEM NATIONAL BCBS CT','CIGNA ALL','CIGNA - GENERIC','CIGNA BEHAVIORAL HEALTH','CIGNA GENERIC','CIGNA HMO','CIGNA MGD MEDICARE','CIGNA OSCAR','CIGNA PPO','OXFORD HEALTH PLANS','AETNA - HMO-MEDICARE AND ALL OTHERS','AETNA HMO','AETNA HMO/POS','AETNA ADVANTAGE PLANS OFF EXCHANGE','AETNA MEDICARE ADVANTAGE',
'AETNA ADVANTAGE PLANS/AETNA OPEN ACCESS') AND CONVERT(int,ROUND(DATEDIFF(hour,patient.dob,mips.dbo.CAT_436_2024.appointmentdate)/8766.0,0))  >= 65) THEN 'Y' ELSE 'N' END) AS PATIENT_MEDICARE_ADVANTAGE
, '436' AS MEASURE_NUMBER
, CAT_436_2024.APPOINTMENTREASON
, MIPS.DBO.HHC_CPT_PIVOT.CPT AS CPT_CODE
, '' as DENOMINATOR_DIAGNOSIS_CODE
, CASE WHEN (report.contenttext LIKE '%reconstruct%' or report.contenttext like '%Iterative%' or report.contenttext like '%Automated exposure control%'
              or addend.contenttext LIKE '%reconstruct%' or addend.contenttext like '%Iterative%' or addend.contenttext like '%Automated exposure control%') THEN 'G9637' ELSE 'G9638' END AS NUMERATOR_RESPONSE_VALUE
...
inner join comm4_HHC.dbo.reportaddendum on report.reportid = reportaddendum.OriginalReportID
inner join comm4_HHC.dbo.report addend on addend.reportid = ReportAddendum.AddendumReportID
Inner join comm4_hhc.dbo.[order] on report.reportid = [order].reportid
inner join comm4_hhc.dbo.visit on [order].visitid = visit.visitid
...
left outer join MIPS.DBO.HHC_CPT_PIVOT ON MIPS.DBO.CAT_436_2024.MCODE = MIPS.DBO.HHC_CPT_PIVOT.[MPI: ID]
						 ...
						 , '73702', '73706', '74150', '74160', '74170', '74174', '74175', '74176', '74177', '74178', '74261', '74262', '74263', '75571', '75572', '75573', '75574', '75635', '76380', '76497'
						 , '77011', '77012', '77013', '77014', '77078', '0042T', 'G0297')
where [ORDER].SITEID = 8  and left([imagingfact].accessionnumber, 2) not in ('CH', 'HM', 'MS', 'SV', 'WH') and departmentdim.departmentcenter = 'Advanced Radiology Partners' AND CONVERT(int,ROUND(DATEDIFF(hour,patient.dob,mips.dbo.CAT_436_2024.appointmentdate)/8766.0,0))>= 18
GROUP BY 
CONVERT(VARCHAR(10), APPOINTMENTDATE, 101), case when CAT_436_2024.READING_RADIOLOGIST = 'Adam Kaye' then '1881838118' 
       when CAT_436_2024.READING_RADIOLOGIST = 'Betty Mathew' then '1093153314'  
	   when CAT_436_2024.READING_RADIOLOGIST = 'Eran Rotem' then '1912294182'  
	   when CAT_436_2024.READING_RADIOLOGIST = 'Joshua Sapire' then '1164452066'  
	   when CAT_436_2024.READING_RADIOLOGIST = 'Kelly Harkins-Squitieri' then '1518915990'  
	   ...
	   when CAT_436_2024.READING_RADIOLOGIST = 'Scott Smith' then '1952496424' 
	   when CAT_436_2024.READING_RADIOLOGIST = 'Terence Hughes' then '1053307421' 
	   else ProviderDim.npi end,CAT_436_2024.Reading_Radiologist, PATIENT.MRN, CONVERT(int,ROUND(DATEDIFF(hour,patient.dob,mips.dbo.CAT_436_2024.appointmentdate)/8766.0,0)), PATIENT.SEX , 
CASE WHEN CoverageDim.PayorFinancialClass IN ('MEDICARE', 'MEDICAREMC') THEN 'Y' ELSE 'N' END, CAT_436_2024.APPOINTMENTREASON,
HHC_CPT_PIVOT.CPT, [ORDER].FILLERORDERNUMBER 
, CASE WHEN (report.contenttext LIKE '%reconstruct%' or report.contenttext like '%Iterative%' or report.contenttext like '%Automated exposure control%'
              or addend.contenttext LIKE '%reconstruct%' or addend.contenttext like '%Iterative%' or addend.contenttext like '%Automated exposure control%') THEN 'G9637' ELSE 'G9638' END
, CAT_436_2024.MCODE
;